...
namespace VECTOR
{
    ...
    */
    void Vect::set_x(double mag,double ang) { x=mag*cos(ang); }
    void Vect::set_y(double mag,double ang) { y=mag*sin(ang); }
    ...
    double Vect::magval() const { return sqrt(x*x+y*y); }
    ...
    std::ostream & operator<<(std::ostream & os,const Vect & v) {
        ...
        else if (v.mode == Vect::POL) os<<"(m,a) = ("<<v.magval()<<","<<v.angval()*RAD_TO_DEG<<")\n";
